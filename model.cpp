...
Model::Model(function<ld(ld, ld)> dzdx_, function<ld(ld, ld)> dzdy_,
    ...
    : dzdx(dzdx_), dzdy(dzdy_), beta(beta_), alpha(alpha_), l(l_) {
  int n = ceil(l / dx);
...
void Model::optimize(size_t iter) {
  int n = Points.size() - 1;
  ...
  for (int t = 0; t < iter; ++t) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      ld J_best_local = std::numeric_limits<ld>::max();
...
ld Model::J(ld x1, ld x2, ld a, ld b) {
  function<ld(ld)> f1{[a, b, dzdx = this->dzdx, dzdy = this->dzdy](ld x) -> ld {
    ld y = a * x + b;
  ...
  ld res = alpha * 0.5 * pow(integrate(f1, x1, x2), 2) + integrate(f2, x1, x2);
  return res;
}
