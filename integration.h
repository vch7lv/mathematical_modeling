...
// 	return (2.0 * cos(2.5 * x) * exp(x / 3.0) + 4 * sin(3.5 * x) * exp(-3 *
// x) + x);
// }
...
// template <typename Number>
// Matrix<Number> RhimannIntegrale(double, double, int,
// Matrix<Number>(*MatrixFunc)(double ksi));

vector<double> makeNodes(int n, double a, double b) {
	vector<double> nodes(n);
	double h = (b - a) / (n - 1);
	nodes[0] = a;
	for (int i = 1; i < n; i++) nodes[i] = nodes[i - 1] + h;
	return nodes;
}
...
Matrix<Number> makeISF(int n, vector<double> &nodes, double a, double b) {
	...
	for (int i = 0; i < nodes.size(); i++) Mu(i, 0) = computeMoment(i, a, b);
...
long double makeCompoundSF(
	...
	for (int i = 0; i < p; i++) {
		if (i != p - 1) {
			Ashki[i] = makeISF<double>(node_counter[i], nodes[i],
									   interv[i].first, interv[i].second);
		} else {
			Ashki[i] = makeISF<double>(node_counter[i], nodes[i],
									   interv[i].first, interv[i].second);
		}
