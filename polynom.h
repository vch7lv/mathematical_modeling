...
template <typename Number> class Polynom {
public:
  size_t n;
  vector<Number> v;
...
template <typename Number> class Polynom<Matrix<Number>> {
public:
  size_t n;
...
template <>
ostream &operator<< <Matrix<double>>(ostream &os,
                                     const Polynom<Matrix<double>> &poly) {
  if (poly.n == 0) {
    os << "Empty poly";
    return os;
  }
